...
class TestHtmlJet(unittest.TestCase):
    ...
    async def test_take_screenshots(self, mock_analyze, mock_playwright):
    ...
    @patch('htmljet.main.async_playwright')
    async def test_take_screenshots_all_levels(self, mock_playwright):
        mock_browser = AsyncMock()
        mock_page = AsyncMock()
        mock_element = AsyncMock()
    ...
    @patch('htmljet.main.async_playwright')
    async def test_analyze_html_structure(self, mock_playwright):
        mock_page = AsyncMock()
        mock_page.content.return_value = "<html><body><div><p>Test</p></div></body></html>"
    ...
    def test_snap_command_all_levels(self, mock_take_screenshots_all_levels):
        mock_take_screenshots_all_levels.return_value = asyncio.Future()
        mock_take_screenshots_all_levels.return_value.set_result(None)
